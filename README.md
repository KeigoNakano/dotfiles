...
## 使用ツール
"chezmoi + mise"
外部パッケージの管理方法はchezmoiexternal.tomlを用いる方法とmiseを用いる方法の２つがあるが、miseのほうが簡潔でgithubのversion非依存。miseで管理できないものについてchezmoiexternal.tomlで管理する方針。
...
miseのtrustと、config.tomlのハッシュ値の変更を検知することでスクリプトを再実行させる。
